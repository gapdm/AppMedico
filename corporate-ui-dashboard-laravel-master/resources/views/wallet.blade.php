<x-app-layout>
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        ...
        <!-- Modal -->
        <div class="modal fade" id="registrarMedicoModal" tabindex="-1" role="dialog" aria-labelledby="registrarMedicoModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="registrarMedicoModalLabel">Registrar Médico</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form id="registrarMedicoForm" action="{{ route('medicos.store') }}" method="POST">
                            @csrf
                            ...
                            <div class="form-group">
                                <label for="tipo_profesion">Tipo de Profesión</label>
                                <input type="text" class="form-control" id="tipo_profesion" name="tipo_profesion" required>
                            </div>
                    ...
                    <div class="card border shadow-xs mb-4">
                        <div class="card-header border-bottom pb-0">
                            <div class="d-sm-flex align-items-center">
                                <div>
                                    <h6 class="font-weight-semibold text-lg mb-0">Lista de medicos</h6>
                                    <p class="text-sm">Informacion de medicos</p>
                                </div>
                                <div class="ms-auto d-flex">
                                    <button type="button" class="btn btn-sm btn-dark btn-icon d-flex align-items-center me-2" data-toggle="modal" data-target="#registrarMedicoModal">
                                        <span class="btn-inner--icon">
                                            <svg width="16" height="16" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="d-block me-2">
                                                <path d="M6.25 6.375a4.125 4.125 0 118.25 0 4.125 4.125 0 01-8.25 0zM3.25 19.125a7.125 7.125 0 0114.25 0v.003l-.001.119a.75.75 0 01-.363.63 13.067 13.067 0 01-6.761 1.873c-2.472 0-4.786-.684-6.76-1.873a.75.75 0 01-.364-.63l-.001-.122zM19.75 7.5a.75.75 0 00-1.5 0v2.25H16a.75.75 0 000 1.5h2.25v2.25a.75.75 0 001.5 0v-2.25H22a.75.75 0 000-1.5h-2.25V7.5z"/>
                                            </svg>
                                        </span>
                                        <span class="btn-inner--text">Registrar medico</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                        <div class="card-body px-0 py-0">
                                                ...
                                                <td>
                                                    <div class="d-flex px-2 py-1">
                                                        <div class="d-flex align-items-center">
                                                            <img src="{{ $medico->avatar_url ?? '../assets/img/team-2.jpg' }}" class="avatar avatar-sm rounded-circle me-2" alt="{{ $medico->nombre }}">
                                                        </div>
                                                        <div class="d-flex flex-column justify-content-center ms-1">
                                                            <h6 class="mb-0 text-sm font-weight-semibold">{{ $medico->nombre }}</h6>
                                                            <p class="text-sm text-secondary mb-0">{{ $medico->email ?? 'No email' }}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $medico->edad }}</p>
                                                </td>
                                                <td class="text-center">
                                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $medico->sexo }}</p>
                                                </td>
                                                <td class="text-center">
                                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $medico->telefono }}</p>
                                                </td>
                                                <td class="text-center">
                                                    <button class="btn btn-icon btn-sm btn-info" onclick="verExpediente({{ $medico->id }})">
                                                        <i class="fas fa-file-alt"></i>
                                                    </button>
                                                    <button class="btn btn-icon btn-sm btn-warning" onclick="modificarmedico({{ $medico->id }})">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    <form action="{{ route('medicos.destroy', $medico->id) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-icon btn-sm btn-danger" onclick="return confirm('¿Estás seguro de eliminar este medico?')">
                            ...
                            <div class="border-top py-3 px-3 d-flex align-items-center">
                                <p class="font-weight-semibold mb-0 text-dark text-sm">
                                    Pagina {{ $medicos->currentPage() }} de {{ $medicos->lastPage() }}
                                </p>
                                <div class="ms-auto">
                                    @if ($medicos->onFirstPage())
                                        <button class="btn btn-sm btn-white mb-0" disabled>Anterior</button>
                                    @else
                                        <a href="{{ $medicos->previousPageUrl() }}" class="btn btn-sm btn-white mb-0">Anterior</a>
                                    @endif
                        
                                    @if ($medicos->hasMorePages())
                                        <a href="{{ $medicos->nextPageUrl() }}" class="btn btn-sm btn-white mb-0">Siguiente</a>
                                    @else
