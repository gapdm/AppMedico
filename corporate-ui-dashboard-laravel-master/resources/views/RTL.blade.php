            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="registrarServicioModalLabel">Registrar Servicio</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form id="registrarServicioForm" action="{{ route('servicios.store') }}" method="POST">
                            @csrf
                    ...
                    <div class="card border shadow-xs mb-4">
                        <div class="card-header border-bottom pb-0">
                            <div class="d-sm-flex align-items-center">
                                <div>
                                    <h6 class="font-weight-semibold text-lg mb-0">Lista de servicios</h6>
                                    <p class="text-sm">Informacion de servicios</p>
                                </div>
                                <div class="ms-auto d-flex">
                                    <button type="button" class="btn btn-sm btn-dark btn-icon d-flex align-items-center me-2" data-toggle="modal" data-target="#registrarServicioModal">
                        ...
                        <div class="card-body px-0 py-0">
                                ...
                                <table class="table align-items-center mb-0">
                                    <thead class="bg-gray-100">
                                        <tr>
                                            ...
                                            <th class="text-center text-secondary text-xs font-weight-semibold opacity-7">Teléfono</th>
                                            <th class="text-center text-secondary text-xs font-weight-semibold opacity-7">Profesión</th>
                                            <th class="text-center text-secondary text-xs font-weight-semibold opacity-7">Tipo de Profesión</th>
                                                ...
                                                <td>
                                                    <div class="d-flex px-2 py-1">
                                                        <div class="d-flex align-items-center">
                                                            <img src="{{ $servicio->avatar_url ?? '../assets/img/team-2.jpg' }}" class="avatar avatar-sm rounded-circle me-2" alt="{{ $servicio->nombre }}">
                                                        </div>
                                                        <div class="d-flex flex-column justify-content-center ms-1">
                                                            <h6 class="mb-0 text-sm font-weight-semibold">{{ $servicio->nombre }}</h6>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $servicio->correo }}</p>
                                                </td>
                                                <td class="text-center">
                                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $servicio->telefono }}</p>
                                                </td>
                                                <td class="text-center">
                                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $servicio->profesion }}</p>
                                                </td>
                                                <td class="text-center">
                                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $servicio->tipo_profesion }}</p>
                                                </td>
                                                <td class="text-center">
                                                    <button class="btn btn-icon btn-sm btn-warning" onclick="modificarservicio({{ $servicio->id }})">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    <form action="{{ route('servicios.destroy', $servicio->id) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-icon btn-sm btn-danger" onclick="return confirm('¿Estás seguro de eliminar este médico?')">
                            ...
                            <div class="border-top py-3 px-3 d-flex align-items-center">
                                <p class="font-weight-semibold mb-0 text-dark text-sm">
                                    Pagina {{ $servicios->currentPage() }} de {{ $servicios->lastPage() }}
                                </p>
                                <div class="ms-auto">
                                    @if ($servicios->onFirstPage())
                                        <button class="btn btn-sm btn-white mb-0" disabled>Anterior</button>
                                    @else
                                        <a href="{{ $servicios->previousPageUrl() }}" class="btn btn-sm btn-white mb-0">Anterior</a>
                                    @endif
                        
                                    @if ($servicios->hasMorePages())
                                        <a href="{{ $servicios->nextPageUrl() }}" class="btn btn-sm btn-white mb-0">Siguiente</a>
                                    @else
